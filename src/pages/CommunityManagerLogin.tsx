...
import { Mail, Lock, LogIn, AlertCircle, Users } from 'lucide-react';
import { useTranslation } from 'react-i18next';
import { loginSchema } from '../schemas';
import { z } from 'zod';
import { useAuth } from '../context/AuthContext';

export const CommunityManagerLogin = () => {
    ...
    <div className="bg-emerald-900 flex items-center justify-center py-12 md:py-20 px-4">
      <div className="bg-white p-6 md:p-10 rounded-3xl shadow-2xl border border-gray-100 w-full max-w-md">
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          <div>
            <label className="block text-sm font-bold text-gray-700 mb-2">{t('cm_login_page.form.email')}</label>
            <div className="relative">
              ...
              <input
                ...
                placeholder="cm@aftras.ci"
              />
            </div>
          </div>

          <div>
            <label className="block text-sm font-bold text-gray-700 mb-2">{t('cm_login_page.form.password')}</label>
            <div className="relative">
          ...
          <button
            disabled={loading}
            type="submit"
            className="w-full bg-emerald-600 text-white py-4 rounded-xl font-bold text-lg hover:bg-emerald-700 transition-all flex items-center justify-center shadow-lg shadow-emerald-600/20"
          >
            {loading ? (
              <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-white" />
            ) : (
              <>
                <LogIn className="w-5 h-5 mr-2" /> {t('cm_login_page.form.submit')}
              </>
            )}
          </button>
        </form>
